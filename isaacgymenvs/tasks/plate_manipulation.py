...
class PlateManipulation(VecTask):
    # Path of the asset folder
    AssetFolder = os.path.join(ROOT_DIR, "assets", "plate_manipulation")
    ...
    def __init__(self, cfg, sim_device, graphics_device_id, headless):
        ...
        self.moduel_num = self.setup["module_num"]
        self.plate_num = self.setup["plate_num"]
        # urdf models
        self.robot_urdf = self.setup["robot_urdf"]
        self.plate_urdf = self.setup["plate_urdf"]
        self.goal_plate_urdf = self.setup["goal_plate_urdf"]
        self.boundary_urdf = self.setup["boundary_urdf"]
        self.table_urdf = self.setup["table_urdf"]
        # boundary and plate information
        self.boundary_diameter = self.setup["boundary_diameter"]
        self.plate_size = self.setup["boundary_diameter"]
        self.boundary_z_offset = self.setup["boundary_z_offset"]
        self.tip_min_position = [-self.boundary_diameter/2, -self.boundary_diameter/2, 0]
        self.tip_min_position = [self.boundary_diameter/2, self.boundary_diameter/2, 0.5]
