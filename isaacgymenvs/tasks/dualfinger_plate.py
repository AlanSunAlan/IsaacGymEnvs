...
class DualfingerPlate(VecTask):
    # Path of the asset folder
    AssetFolder = os.path.join(ROOT_DIR, "assets", "dualfinger_plate")
    ...
    FingertipFrameNames = ["0-fingertip_frame", "1-fingertip_frame"]
    ...
    ModuleNum = 2 # How many 3DoF modules do you use

    # Control related parameters
    # Kp and Kd gains for each DoF in a single module
    ModuleKp = [1.2, 1.2, 1.2] # Equivalent to stiffness
    ModuleKd = [0.06, 0.06, 0.06] # Equivalent to damping
    # Joint velocity limits
    ModuleMaxJointVelocites = [3, 3, 3] # In rad/s
    ...
    def __init__(self, cfg, sim_device, graphics_device_id, headless):
        ...
        # Plate state buffer
        self.plate_state_buf = Deque(maxlen=2) # storing only current state and last state of the plate
    ...
    def computeObservation(self):
        ...
        # Get joint angles
        self.tip_states = self.rb_state_tensor[:, self.tip_indices]
        ...
        # Save the current state to the buffer
        self.plate_state_buf.append(self.plate_2d_state[:, 0:3])
    ...
    def computeReward(self):
        ...
        # Plate moving reward
        plate_moving_reward = torch.zeros_like(dis_l2)
        plate_rotating_reward = torch.zeros_like(dis_l2)
        if len(self.plate_state_buf) == 2:
            # enough history to compare
            delta_state = self.plate_state_buf[1] - self.plate_state_buf[0] # Current - last
            delta_distance = torch.norm(delta_state[:, 0:2], p=2, dim=-1)

            delta_yaw = torch.abs(delta_state[:, 2])
            
            plate_moving_reward = self.cfg["env"]["reward_weight"]["plate_moving_weight"] * delta_distance
            plate_rotating_reward = self.cfg["env"]["reward_weight"]["plate_rotating_weight"] * delta_yaw

        # Fingertip staying near the plate reward
        fingertip_z_values = self.tip_states[:, :, 2]
        z_reward_thres = self.cfg["env"]["reward_weight"]["z_value_thresh"] # Agent can receive reward
        z_reward_thres_tensor = torch.tensor([z_reward_thres], device=self.device).repeat(fingertip_z_values.shape)
        fingertip_below_thresh = fingertip_z_values < z_reward_thres_tensor
        # Sum over -1 dimension
        fingertip_below_sum = torch.sum(fingertip_below_thresh, dim=-1)
        fingertip_below_reward = fingertip_below_sum * self.cfg["env"]["reward_weight"]["z_value_reward"]

        dis_weighted = self.cfg["env"]["reward_weight"]["dis_weight"] * dis_l2
        yaw_weighted = self.cfg["env"]["reward_weight"]["yaw_weight"] * min_yaw_diff
        self.rew_buf =  dis_weighted + yaw_weighted + plate_moving_reward + plate_rotating_reward + fingertip_below_reward
        #print(self.rew_buf) 

        # Touch plate reward
        # If the plate moves, give the agent some reward
        #print("{}".format(fingertip_below_reward[0]))
