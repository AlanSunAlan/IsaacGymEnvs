...
class QuadfingerPlate(VecTask):
    ...
    AssetFolder = os.path.join(ROOT_DIR, "quadfinger_plate")

    # URDF file path
    RobotURDF = os.path.join(AssetFolder, "gripper.urdf")
    GoalObjectURDF = os.path.join(AssetFolder, "plate.urdf")
    BoundaryURDF = os.path.join(AssetFolder, "boundary.urdf")

    # Some important parameters specific to each task
    ModuleNum = 4 # How many 3DoF modules do you use

    # Observation space calculation
    ObsDimensions = {"joint_angles": ModuleNum,
                     "joint_velocities": ModuleNum,
                     "tip_state": 13 , # Including: position, orientation (quaternion), linear and angular velocity
                     "goal_object_pose": 7 , # Including position and quaternion
                     "current_object_state": 13, 
                     }
